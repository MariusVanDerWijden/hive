...
func VersionedHashesFromBlobBundle(bb *e_typ.BlobsBundle, commitmentVersion byte) ([]common.Hash, error) {
	if bb == nil {
...
func (blobId BlobID) VerifyBlob(blob *e_typ.Blob) (bool, error) {
	if blob == nil {
	...
	if blobId == 0 {
		...
		for chunkIdx := 0; chunkIdx < e_typ.FieldElementsPerBlob; chunkIdx++ {
...
func (blobId BlobID) FillBlob(blob *e_typ.Blob) error {
	if blob == nil {
...
func (blobId BlobID) GenerateBlob() (*e_typ.Blob, *e_typ.KZGCommitment, error) {
	blob := e_typ.Blob{}
	...
	typesKzgCommitment := e_typ.KZGCommitment(kzgCommitment)
...
func BlobDataGenerator(startBlobId BlobID, blobCount uint64) ([]common.Hash, *e_typ.BlobTxWrapData, error) {
	blobData := e_typ.BlobTxWrapData{
		Blobs:       make(e_typ.Blobs, blobCount),
		Commitments: make([]e_typ.KZGCommitment, blobCount),
	}
...
func (tc *BlobTransactionCreator) MakeTransaction(nonce uint64) (e_typ.Transaction, error) {
	...
	return &e_typ.TransactionWithBlobData{
		Tx:       signedTx,
		BlobData: blobData,
	}, nil
}
